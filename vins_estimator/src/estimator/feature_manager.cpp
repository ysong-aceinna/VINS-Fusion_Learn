...
bool FeatureManager::addFeatureCheckParallax(int frame_count, const map<int, vector<pair<int, Eigen::Matrix<double, 7, 1>>>> &image, double td)
{
    ...
    {
        DLOG(INFO) << "parallax_sum: " << parallax_sum << " ,parallax_num: " << parallax_num;
        DLOG(INFO) << "current parallax: " << parallax_sum / parallax_num * FOCAL_LENGTH;
...
void FeatureManager::setDepth(const VectorXd &x)
{
    ...
    for (auto &it_per_id : feature)
    {
        ...
        // LOG(INFO) << "feature id " << it_per_id->feature_id << " ,start_frame " << it_per_id-> start_frame << " ,depth " << it_per_id->estimated_depth;
...
void FeatureManager::triangulate(int frameCnt, Vector3d Ps[], Matrix3d Rs[], Vector3d tic[], Matrix3d ric[])
{
    for (auto &it_per_id : feature)
    {
        ...
        LOG_IF(ERROR, !(svd_idx == svd_A.rows())) << "svd_idx == svd_A.rows()";
